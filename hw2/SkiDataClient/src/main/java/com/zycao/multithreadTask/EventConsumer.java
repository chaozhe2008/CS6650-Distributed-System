...
class EventConsumer implements Runnable {
    ...
    private int firstPhaseRequests;
    private final HttpClient httpClient;

    EventConsumer(HttpClient httpClient, BlockingQueue<SkierLiftRideEvent> queue, RequestCounter counter, CountDownLatch initialLatch, CountDownLatch totalRequestsLatch, int firstPhaseRequests) {
    ...
    private Integer sendPostRequest(SkierLiftRideEvent event) throws JsonProcessingException {
    ...
    public void run() {
        while (totalRequestsLatch.getCount() > 0) {
            try {
                ...
                if (++requestCount >= firstPhaseRequests) {
